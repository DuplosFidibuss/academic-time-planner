...
Local Storage is an open-source library for Blazor, providing functionality to store application data in the browser cache. This storage is unique to every browser session. That is to say, if the same application is opened in multiple tabs within the same browser window, they all access the same local storage.